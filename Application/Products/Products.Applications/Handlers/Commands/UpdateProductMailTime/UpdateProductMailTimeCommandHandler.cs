...
internal class UpdateProductMailTimeCommandHandler : IRequestHandler<UpdateProductMailTimeCommand, GetProductDto>
{
    ...
    public UpdateProductMailTimeCommandHandler(IBaseWriteRepository<Product> products, IMapper mapper, ICurrentUserService currentUserService, ICleanProductsCacheService cleanProductsCacheService)
    {
        _products = products;
        _mapper = mapper;
    ...
    public async Task<GetProductDto> Handle(UpdateProductMailTimeCommand request, CancellationToken cancellationToken)
    {
